...
public:
     ...
     */
    inline float level() { if(word == 0.0) throw string("[LEVEL_ERR]: This file only contains empty sentences!"); return 0.0588*Ldoc() - 0.296*Sdoc() - 15.8 ; }
    inline float Ldoc() { return (letter/word) * 100.0; }
    inline float Sdoc() { return (sentence/word) * 100.0; }
private:
    float letter;
    float word;
    float sentence;
